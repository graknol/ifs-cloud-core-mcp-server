...
END;''',
            ...
            'ast_info': {
                'control_structures': [
                    {'type': 'SELECT', 'condition': 'qty_onhand FROM inventory_part_tab'},
...
END;''',
            ...
            'ast_info': {
                'control_structures': [
                    {'type': 'FOR', 'condition': 'rec IN (SELECT * FROM work_order_operation'},
    ...
    for proc in test_procedures:
        ...
        if 'control_structures' in proc.get('ast_info', {}):
            for structure in proc['ast_info']['control_structures']:
                business_parts.append(f"-- {structure['type']}: {structure.get('condition', '')}")
        
        # Add simplified body
        lines = proc['body'].split('\n')
        code_lines = [line for line in lines[1:6] if line.strip()]  # Skip BEGIN, take next 5 lines
...
def test_gui_only():
    ...
    try:
        ...
        test_data[0]['generated_summary'] = "Calculates customer discount rates based on order amount with tiered discount structure"
        test_data[0]['human_summary'] = test_data[0]['generated_summary']
        test_data[0]['status'] = 'pending'
        
        test_data[1]['generated_summary'] = "Validates inventory levels for parts at specific sites against required quantities"
        test_data[1]['human_summary'] = test_data[1]['generated_summary'] 
        test_data[1]['status'] = 'pending'
        
        test_data[2]['generated_summary'] = "Processes work order operations by updating status and logging employee progress"
...
def test_validation_system():
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            validator.training_history['val_loss'] = [1.5, 1.2, 1.0, 1.1, 1.2, 1.3]  # Simulate overfitting
