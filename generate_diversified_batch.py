...
class DiversifiedBatchGenerator:
    ...
    def extract_procedures_from_file(self, file_path: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for match in matches:
                ...
                end_line_idx = min(
                    start_line_idx + 200, len(content_lines)
                )  # Max 200 lines per procedure

                # Try to find proper END statement
                for i in range(
                    start_line_idx + 10, min(start_line_idx + 200, len(content_lines))
                ):
                    line = content_lines[i].strip().upper()
                    if (
                        line.startswith("END ") and proc_name.upper() in line
                    ) or line == "END;":
                        end_line_idx = i + 1
                        break

                procedure_content = "\n".join(
                    content_lines[start_line_idx:end_line_idx]
                )
    ...
    def calculate_complexity(self, content: str) -> float:
        ...
        complexity = sum(
            count * weights[indicator] for indicator, count in indicators.items()
        )
    ...
    def create_diversified_plan(self) -> List[Dict[str, Any]]:
        ...
        quality_procedures = [
            ...
            if p["complexity_score"] > 5.0  # Minimum complexity
            and p["content_length"] > 500  # Minimum size
            and p["content_length"] < 12000
        ]  # Maximum size for context window
        ...
        module_quotas = {
            module: target_count // len(TARGET_MODULES) for module in TARGET_MODULES
        }
        ...
        for module in TARGET_MODULES:
            ...
            if len(module_procs) >= quota:
                ...
                top_complex = module_procs[
                    : min(quota * 3, len(module_procs))
                ]  # Top candidates
                selected = random.sample(top_complex, quota)
                selected_procedures.extend(selected)
                print(
                    f"  {module}: Selected {len(selected)}/{len(module_procs)} procedures"
                )
            else:
                # Take all available
                selected_procedures.extend(module_procs)
                print(
                    f"  {module}: Selected {len(module_procs)}/{len(module_procs)} procedures (all available)"
                )
        ...
        for i, proc in enumerate(
            tqdm(selected_procedures, desc="Pre-computing contexts")
        ):
...
**Summary:**"""
    ...
    def run_diversified_generation(self):
        ...
        with torch.no_grad():
            for i, planning_item in enumerate(
                ...
                if i > 0 and i % 5 == 0 and torch.cuda.is_available():
                    ...
                    memory_reserved = torch.cuda.memory_reserved() / 1024**3
                    print(
                        f"🧹 Memory cleared at {i}: Allocated: {memory_allocated:.2f}GB, Reserved: {memory_reserved:.2f}GB"
                    )
                ...
                try:
                    ...
                    if summary:
                        ...
                        tokens_per_sec = (
                            500 / proc_time if proc_time > 0 else 0
                        )  # Updated token rate
