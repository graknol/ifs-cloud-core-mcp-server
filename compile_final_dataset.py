...
class FinalDatasetCompiler:
    def __init__(self):
        ...
        self.abbreviation_indicators = [
            ...
            lambda w: not any(vowel in w for vowel in 'aeiou') and len(w) < 5,  # No vowels and short
        ]
            ...
            for row in reader:
                ...
                if similarity_score > 0.58:
                    high_quality_matches.append({
                        ...
                        'match_type': action.split(' -')[0] if ' -' in action else action
                    })
                        
        print(f"🎯 Found {len(high_quality_matches)} high-quality clustered matches (score > 0.58)")
        ...
        for match_data in clustered_matches:
            ...
            for group_key in self.word_groups:
                ...
                for group_word_data in self.word_groups[group_key]:
                    if self.calculate_word_similarity(word, group_word_data['word']) > best_similarity:
                        best_similarity = self.calculate_word_similarity(word, group_word_data['word'])
        ...
        for group_key, word_list in self.word_groups.items():
            ...
            if best_word_data:
                ...
                for w in word_list:
                    if w['word'] != best_word_data['word']:
                        if 'similarity_score' in w:
                            variants_with_scores.append(f"{w['word']}({w['similarity_score']:.2f})")
                        else:
                ...
                self.final_keywords[best_word_data['word']] = {
                    ...
                    'variants': [w['word'] for w in word_list if w['word'] != best_word_data['word']],
                    'variants_with_scores': variants_with_scores,
                    'variant_count': len([w for w in word_list if w['word'] != best_word_data['word']]),
    ...
    def export_final_dataset(self, output_file):
        ...
        with open(output_file, 'w', newline='', encoding='utf-8') as f:
            writer = csv.writer(f)
            writer.writerow([
                'keyword', 'total_occurrences', 'primary_occurrences', 
                'variant_count', 'variants_with_scores', 'sources', 'notes'
            ])
            ...
            for keyword, data in sorted_keywords:
                writer.writerow([
                    ...
                    data['variant_count'],
                    ', '.join(data['variants_with_scores'][:10]),  # Show variants with scores
                    ', '.join(data['sources']),
                    data['notes']
                ])
    ...
    def show_dataset_preview(self, top_n=30):
        ...
        for i, (keyword, data) in enumerate(sorted_keywords[:top_n], 1):
            variants_str = f" + {data['variant_count']} variants" if data['variant_count'] > 0 else ""
            sources_str = ",".join(data['sources'])
            
            print(f"{i:2d}. {keyword:<15} ({data['total_occurrences']:,} occur.){variants_str} [{sources_str}]")
            
            if data['variants_with_scores']:
                preview_variants = data['variants_with_scores'][:5]
                if len(data['variants_with_scores']) > 5:
                    preview_variants.append(f"...+{len(data['variants_with_scores'])-5} more")
    ...
    def generate_summary_stats(self):
        ...
        total_occurrences = sum(data['total_occurrences'] for data in self.final_keywords.values())
        
        curated_count = sum(1 for data in self.final_keywords.values() if 'curated' in data['sources'])
        clustered_count = sum(1 for data in self.final_keywords.values() if 'clustered' in data['sources'])
        both_count = sum(1 for data in self.final_keywords.values() if len(data['sources']) > 1)
        
        total_variants = sum(data['variant_count'] for data in self.final_keywords.values())
