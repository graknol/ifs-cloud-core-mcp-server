...
class WordClusterer:
    ...
    def load_csv_data(self):
        ...
        print(f"📊 Loaded {len(self.insightful_words)} insightful words from first 1005 rows")
    ...
    def calculate_similarity_score(self, word1, word2):
        ...
        if word1 in word2 or word2 in word1:
            containment_score = min(len(word1), len(word2)) / max(len(word1), len(word2))
    ...
    def generate_clustering_report(self, clusters, output_file):
        ...
        with open(output_file, 'w', newline='', encoding='utf-8') as f:
            writer = csv.writer(f)
            writer.writerow([
                'unchecked_word', 'occurrences', 'best_match', 'similarity_score',
                'match_occurrences', 'match_notes', 'sequence_sim', 'containment',
                'prefix_suffix', 'char_overlap', 'abbreviation', 'suggested_action'
            ])
    ...
    def show_clustering_preview(self, clusters, top_n=20):
        ...
        for i, (unchecked_word, cluster_data) in enumerate(sorted_items[:top_n], 1):
            ...
            print(f"    → {best_match['insightful_word']:<15} (similarity: {best_match['score']:.3f})")
    ...
    def update_csv_with_suggestions(self, clusters, output_csv):
        ...
        for unchecked_word, cluster_data in clusters.items():
            ...
            if (breakdown['abbreviation'] > 0.5 or 
                breakdown['containment'] > 0.8 or 
                best_match['score'] > 0.85):
