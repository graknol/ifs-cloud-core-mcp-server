...
**Summary:**'''
            ...
            inputs = self.tokenizer(prompt, return_tensors="pt", padding=True, truncation=True, max_length=6000)
    ...
    def run_batch_generation(self):
        ...
        with torch.no_grad():
            for file_path in tqdm(plsql_files[:target_summaries], desc="Processing files"):
                ...
                if processed_count % 20 == 0 and processed_count > 0 and torch.cuda.is_available():
                    torch.cuda.empty_cache()
                ...
                try:
                    ...
                    if content_info and len(content_info['content']) > 100:  # Skip tiny files
                        ...
                        if summary:
                            ...
                            tokens_per_sec = 700 / proc_time if proc_time > 0 else 0  # Updated token rate
        ...
        total_time = time.time() - start_time
        avg_tokens_per_sec = (processed_count * 700) / total_time if total_time > 0 else 0
