...
with open('batch_summaries/correlated_summaries_with_prompts_20250826_101247.json', 'r', encoding='utf-8') as f:
    data = json.load(f)
...
for module in sorted(modules.keys()):
    ...
    print(f'     - Total tokens: {(stats["summary_tokens"] + stats["prompt_tokens"]):,}')
...
print()
print('🔍 EXTREMES:')
print(f'   Largest summary: {max_summary["summary_token_count"]} tokens ({max_summary["module"]}.{max_summary["procedure_name"]})')
print(f'   Smallest summary: {min_summary["summary_token_count"]} tokens ({min_summary["module"]}.{min_summary["procedure_name"]})')
print(f'   Largest prompt: {max_prompt["prompt_token_count"]} tokens ({max_prompt["module"]}.{max_prompt["procedure_name"]})')
print(f'   Smallest prompt: {min_prompt["prompt_token_count"]} tokens ({min_prompt["module"]}.{min_prompt["procedure_name"]})')
