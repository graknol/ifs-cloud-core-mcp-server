...
class OptimizedBatchGenerator:
    ...
    def load_model_full_gpu(self):
        ...
        try:
            ...
            logger.info(f"Tokenizer setup: pad_token='{self.tokenizer.pad_token}', eos_token='{self.tokenizer.eos_token}'")
            ...
            if torch.cuda.is_available():
                ...
                logger.info(f"GPU Memory: {memory_used:.2f}GB / {memory_total:.2f}GB ({memory_used/memory_total*100:.1f}%)")
    ...
    def extract_procedures_properly(self) -> List[Dict]:
        ...
        try:
            ...
            logger.info(f"✅ Extracted {len(limited_procedures)} procedures (from {len(procedures)} total)")
            
            # Log sample of what we extracted
            if limited_procedures:
                sample = limited_procedures[0]
                logger.info(f"Sample procedure: {sample.get('name', 'Unknown')} from {sample.get('module', 'Unknown')}")
                logger.info(f"Has parameters: {len(sample.get('parameters', []))} params")
    ...
    def create_enhanced_prompt(self, procedure: Dict) -> str:
        ...
        enhanced_context = self.training_loop.enhance_context_with_keywords(context, procedure.get('name', ''))
...
Business Summary:"""
    ...
    def generate_summary_optimized(self, procedure: Dict) -> str:
        ...
        try:
            ...
            encoding = self.tokenizer(
                prompt,
                return_tensors="pt",
                max_length=min(self.config["max_length"], 3072),  # Leave room for generation
                truncation=True,
    ...
    def find_existing_progress(self) -> tuple[List[Dict], int]:
        ...
        try:
            ...
            logger.info(f"📂 Found existing progress: {start_index} summaries in {latest_progress.name}")
            
            # Update output file to continue the series
            self.output_file = latest_progress.parent / latest_progress.name.replace("_progress_", "_continued_")
    ...
    def run_optimized_generation(self):
        ...
        logger.info(f"🎯 Generating {remaining} remaining summaries (starting from #{start_index + 1})")
        ...
        for i in range(start_index, min(target_count, len(procedures))):
            try:
                ...
                logger.info(f"Processing {i+1}/{target_count}: {procedure.get('name', 'Unknown')}")
        ...
        logger.info(f"✅ Optimized generation complete! Created {len(results)} summaries")
...
def main():
    ...
    try:
        ...
        if results:
            print(f"🎉 Success! Generated {len(results)} summaries with full GPU optimization")
