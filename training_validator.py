...
class TrainingValidator:
    ...
    def load_validation_history(self):
        ...
        if history_file.exists():
            ...
            logger.info(
                f"Loaded validation history: {len(self.training_history['iterations'])} iterations"
            )
    ...
    def create_validation_set(
        self, all_procedures: List[Dict], current_summaries: List[Dict]
        ...
        logger.info(
            f"Created train/val split: {len(train_summaries)} train, {len(val_summaries)} val"
        )

        return train_summaries, val_summaries

    def should_continue_training(
        self, current_iteration: int, val_loss: float
    ) -> Tuple[bool, str]:
        """
        ...
        if val_loss < self.training_history["best_val_loss"] - self.min_delta:
            ...
            return (
                True,
                f"No improvement ({self.training_history['patience_counter']}/{self.patience})",
            )

    def evaluate_model(
        self, model, tokenizer, validation_summaries: List[Dict]
    ) -> float:
        """
        ...
        with torch.no_grad():
            for summary_data in validation_summaries:
                ...
                messages = [
                    {
                        "role": "system",
                        "content": "You are an expert at analyzing IFS Cloud business procedures.",
                    },
...
Provide a single sentence summary that describes the business purpose of this procedure."""
    ...
    def plot_training_curves(self, save_path: Optional[str] = None):
        ...
        if (
            "train_loss" in self.training_history
            and self.training_history["train_loss"]
        ):
            plt.plot(
                iterations,
    ...
    def create_training_report(self) -> str:
        ...
        report.append(
            f"  • Total iterations: {len(self.training_history['iterations'])}"
        )
        report.append(f"  • Validation set size: {len(self.validation_procedures)}")
        report.append(
            f"  • Best validation loss: {self.training_history['best_val_loss']:.4f}"
        )
        report.append(
            f"  • Patience counter: {self.training_history['patience_counter']}/{self.patience}"
        )
        report.append("")
