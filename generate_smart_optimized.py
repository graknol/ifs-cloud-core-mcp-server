#!/usr/bin/env python3
"""
Optimized Batch Generator with Smart Memory Management
Uses CPU offloading initially, then moves to GPU for inference
"""
...
class SmartBatchGenerator:
    """Batch generator with smart memory management."""
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.output_file = self.output_dir / f"smart_optimized_summaries_{timestamp}.json"
    ...
    def load_model_smart_gpu(self):
        ...
        try:
            ...
            # Load model with auto device mapping (allows CPU offloading)
            model_kwargs = {
                "torch_dtype": torch.float16,
                "device_map": "auto",  # Let transformers decide optimal placement
                "trust_remote_code": True,
                "use_cache": True,
                "low_cpu_mem_usage": True,  # Enable CPU offloading when needed
    ...
    def extract_procedures(self, file_path: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            parsed_info = self.parser.parse_code(content)
            
            procedures = []
            if parsed_info and isinstance(parsed_info, dict) and 'procedures' in parsed_info:
                ...
                # Fallback - use the whole file content as one "procedure"
                procedures.append({
...
**Summary:**'''
    ...
    def run_batch_generation(self):
        ...
        logger.info("🚀 Starting smart optimized batch generation...")
                ...
                for proc in procedures:
                    if len(proc['content']) > 100:  # Skip tiny procedures
                        ...
                        if summary:
                            ...
                            tokens_per_sec = 400 / proc_time if proc_time > 0 else 0  # Approximate token rate
        ...
        total_time = time.time() - start_time
        avg_tokens_per_sec = (processed_count * 400) / total_time if total_time > 0 else 0
...
def main():
    ...
    print("\n🎯 Smart Optimized IFS Cloud Batch Summary Generator")
    print("=" * 65)
    ...
    print(f"  Mode: Smart GPU/CPU distribution")
    ...
    # Run generation
    generator = SmartBatchGenerator(config)
